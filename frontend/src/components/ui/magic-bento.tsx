...
export const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      gridRef.current?.querySelectorAll('.card--border-glow').forEach(card => (card as HTMLElement).style.setProperty('--glow-intensity', '0'));
...
export const BentoCardGrid: React.FC<{ children: React.ReactNode; gridRef?: React.RefObject<HTMLDivElement | null>; className?: string; }> = ({ children, gridRef, className = '' }) => (
  <div className={`bento-section grid gap-2 p-3 select-none relative ${className}`} style={{ fontSize: 'clamp(1rem, 0.9rem + 0.5vw, 1.5rem)' }} ref={gridRef}>
    {children}
  </div>
);

export const BentoStyles = ({ glowColor = DEFAULT_GLOW_COLOR }: { glowColor?: string }) => (
  <style>
    {`
      .bento-section {
        --glow-x: 50%;
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {cardData.map((card, index) => (
            <ParticleCard
              key={index}
              className={`card flex flex-col justify-between relative min-h-[200px] p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 ${enableBorderGlow ? 'card--border-glow' : ''}`}
              style={{ backgroundColor: card.color || 'var(--background-dark)', borderColor: 'var(--border-color)', color: 'var(--white)' }}
            ...
            >
              ...
              <div className="card__content flex flex-col relative z-10 text-white mt-auto">
